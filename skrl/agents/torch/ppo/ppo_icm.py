...
class ICM(nn.Module):
    ...
    def __init__(self, observation_space: gymnasium.Space, action_space: gymnasium.Space, 
                 feature_dim: int = 256, device: Union[str, torch.device] = "cpu"):
        ...
        self.forward_model = nn.Sequential(
            nn.Linear(feature_dim + action_space.shape[0], 256),
            nn.ReLU(),
            nn.Linear(256, feature_dim)
        ).to(device)
        
    def forward(self, states: torch.Tensor, next_states: torch.Tensor, actions: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        intrinsic_reward = 0.5 * F.mse_loss(predicted_phi_s_next, phi_s_next.detach(), reduction='none').mean(dim=1, keepdim=True)
    ...
    def compute_loss(self, states: torch.Tensor, next_states: torch.Tensor, 
                    actions: torch.Tensor, forward_scale: float = 0.2, inverse_scale: float = 0.8) -> Dict[str, torch.Tensor]:
        """Compute ICM losses"""
            ...
            if self.icm_enabled:
                with torch.no_grad():
                    icm_outputs = self.icm(
                        ...
                        self._state_preprocessor(next_states),
                        actions
                    )
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        for epoch in range(self._learning_epochs):
            ...
            for batch in sampled_batches:
                if self.icm_enabled:
                    (sampled_states, sampled_actions, sampled_log_prob, sampled_values, 
                     sampled_returns, sampled_advantages, sampled_next_states, sampled_intrinsic_rewards) = batch
                else:
                    (sampled_states, sampled_actions, sampled_log_prob, 
                     sampled_values, sampled_returns, sampled_advantages) = batch
                ...
                with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                    ...
                    icm_loss = 0
                    if self.icm_enabled:
                        sampled_next_states_preprocessed = self._state_preprocessor(sampled_next_states, train=not epoch)
                        icm_losses = self.icm.compute_loss(
                            sampled_states, sampled_next_states_preprocessed, sampled_actions,
                            self._icm_forward_loss_scale, self._icm_inverse_loss_scale
                        )
        ...
        if self.icm_enabled:
            ...
            self.track_data("Reward / Intrinsic", sampled_intrinsic_rewards.mean().item())
            self.track_data("Reward / Extrinsic", (rewards - sampled_intrinsic_rewards).mean().item())
